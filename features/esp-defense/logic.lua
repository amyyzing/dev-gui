...
local function isReceiverClosed(receiverPlayer,carrierData,defenders,app,interceptionCore)
				...
				if character then
					if shouldHighlightPlayer(player) then
						...
						elseif carrierData and isSameTeam(player,carrierData.player) then
							if isReceiverClosed(player,carrierData,defenders,app,interceptionCore) then
