...
OPTIONAL_MODULE_PATH_SET={
	[MODULE_PATHS.ExternalRenderer]=true,
...
function loadEmbeddedModule(modulePath)
	if not isAllowedModulePath(modulePath) then
		warn("Blocked embedded module path:",modulePath)
		return nil
	end
	...
	local source=type(EMBEDDED_MODULE_SOURCES)=="table" and EMBEDDED_MODULE_SOURCES[modulePath]
	if type(source)~="string" or source=="" then
		REMOTE_MODULE_SOURCES[modulePath]=false
		warn("Optional embedded module unavailable:",modulePath)
		return nil
	end

	local module,err=loadModuleFromSource(modulePath,source)
	if not module then
		warn("Embedded module failed while loading:",modulePath,err)
		return nil
	end
...
function loadEmbeddedModuleStep(name,path)
	loaderCurrent+=1
	setLoaderProgress("Fetching "..path,loaderCurrent-0.35,LOADER_TOTAL,false)
	local module=loadEmbeddedModule(path)
	setLoaderProgress((module and "Loaded " or "Missing ")..path,loaderCurrent,LOADER_TOTAL,not module)
	return module
end
...
ExternalRendererModule=loadEmbeddedModuleStep("ExternalRenderer",MODULE_PATHS.ExternalRenderer)
...
UILibRayfieldModule={
	...
	Components={TextFont=Enum.Font.Gotham,TitleFont=Enum.Font.GothamBold,ControlFont=Enum.Font.GothamMedium,SectionMode="label",SectionPrefix=false,SectionPaddingX=14,SectionPaddingY=12,SectionGap=8,SectionHeaderHeight=21,SectionTitleSize=13,SectionSubtitleSize=11,SectionBackgroundTransparency=1,SectionStrokeTransparency=1,SectionBodyInset=0,SectionBodyGap=8,SliderLabelPlacement="above",SliderRowHeight=54,SliderTrackWidth=224,SliderLabelWidth=108,SliderLabelHeight=14,SliderLabelY=4,SliderControlGap=5,SliderBottomPadding=7,SliderTrackGap=12,SliderValueBoxWidth=52,SliderValueBoxGap=10,SliderValueBoxHeight=28,SliderValueBoxVisible=true,SliderContainerTransparency=0.04,SliderContainerStrokeTransparency=0.8,SliderContainerRole="BUTTON",SliderContainerCornerRole="Control",SliderTrackStrokeTransparency=0.82,SliderValueBoxStrokeTransparency=0.9,SliderLabelX=14,SliderRightPadding=10,ToggleWidth=54,ToggleHeight=24,ToggleKnobSize=18,TogglePad=3,ToggleStyle="pill",ToggleOffRole="INPUT",ToggleStrokeTransparency=0.82,TextBoxHeight=30,ButtonHeight=32,ControlStrokeTransparency=0.78},
	MainFrame={Window={W=500,H=480,MinW=420,MinH=360,MaxW=900,MaxH=760,StartY=80,MinimizedH=60},Layout={RootPadding=0,MainGap=8,PageGap=8,ColumnGap=8,FooterGap=8,HeaderHeight=46,PageBarHeight=44,PageTabWidth=96,PageTabHeight=30,PageHostReserve=142,FooterHeight=34,TopButtonSize=28,TopButtonGap=6,TopButtonOuter=10,FabSize=42,PageShellTransparency=1,PageSliderTransparency=0,HeaderSubtitleVisible=false,HeaderTitleY=14,HeaderTitleSize=15,RootStrokeTransparency=0.38,HeaderStrokeTransparency=0.46,PageShellStrokeTransparency=1,PageSliderStrokeTransparency=0.82}},
...
UILibLinoriaModule={
	...
	Shape={WindowRadius=0,SectionRadius=0,ControlRadius=0,SliderRadius=0,SliderHeight=16,SliderStyle="thin",WindowStrokeTransparency=0.26,SectionStrokeTransparency=0.36,ControlStrokeTransparency=0.38,SliderStrokeTransparency=0.42,AccentStrokeTransparency=0.42},
	Components={TextFont=Enum.Font.Code,TitleFont=Enum.Font.Code,ControlFont=Enum.Font.Code,SectionMode="groupbox",SectionPrefix=false,SectionPaddingX=8,SectionPaddingY=8,SectionGap=3,SectionHeaderHeight=18,SectionTitleSize=12,SectionSubtitleSize=10,SectionTitleBoxWidth=140,SectionTitleBoxHeight=16,SectionTitleOffsetY=-2,SectionBackgroundTransparency=0,SectionStrokeTransparency=0.36,SectionBodyInset=0,SectionBodyGap=4,SliderLabelPlacement="above",SliderRowHeight=38,SliderTrackWidth=210,SliderLabelWidth=116,SliderLabelHeight=12,SliderLabelY=2,SliderControlGap=2,SliderBottomPadding=4,SliderValueBoxWidth=48,SliderValueBoxGap=5,SliderValueBoxHeight=18,SliderValueBoxVisible=true,SliderContainerTransparency=1,SliderContainerStrokeTransparency=1,SliderTrackStrokeTransparency=0.42,SliderValueBoxStrokeTransparency=0.42,SliderLabelX=8,SliderRightPadding=6,ToggleWidth=14,ToggleHeight=14,ToggleKnobSize=10,TogglePad=2,ToggleStyle="checkbox",ToggleOffRole="INPUT",ToggleStrokeTransparency=0.38,ToggleRowHeight=22,TextBoxHeight=24,ButtonHeight=26,ControlStrokeTransparency=0.38},
	MainFrame={Window={W=760,H=560,MinW=560,MinH=380,MaxW=1120,MaxH=760,StartY=70,MinimizedH=44},Layout={RootPadding=6,MainGap=4,PageGap=6,ColumnGap=6,FooterGap=6,HeaderHeight=32,PageBarHeight=24,PageTabWidth=92,PageTabHeight=22,PageHostReserve=108,FooterHeight=28,TopButtonSize=22,TopButtonGap=4,TopButtonOuter=8,FabSize=36,PageShellTransparency=0,PageSliderTransparency=0,HeaderSubtitleVisible=false,HeaderTitleY=8,HeaderTitleSize=14,RootStrokeTransparency=0.2,HeaderStrokeTransparency=0.3,PageShellStrokeTransparency=0.32,PageSliderStrokeTransparency=0.35}},
...
function applyAutoRefreshModuleChange(changedPath,module)
	...
	if changedPath==MODULE_PATHS.ExternalRenderer then
		ExternalRendererModule=module

		if refreshExternalRenderer then
			warn("Auto-refreshing external renderer after remote change:",changedPath)
			task.defer(refreshExternalRenderer)
		else
			warn("Auto-refresh cached external renderer, rebuild not ready:",changedPath)
		end
...
function startAutoRefresh()
	...
	AutoRefreshAPI=AutoRefreshModule.new({
		...
		getRemoteSource=function(path)
			if path==MODULE_PATHS.ExternalRenderer then
				local source=type(EMBEDDED_MODULE_SOURCES)=="table" and EMBEDDED_MODULE_SOURCES[path]
				if type(source)~="string" or source=="" then
					return nil,"embedded external renderer source missing"
				end

				local verified,verifyErr=verifyRemoteModuleSource(path,source)
				if not verified then
					return nil,verifyErr
				end
