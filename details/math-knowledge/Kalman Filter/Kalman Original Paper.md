...
Converted from: `Kalman Filter\Kalman Original Paper.pdf`
...
(1) The optimal filter is specifie d by its impulse response. It is
...
problems, sidestepping the diffic ulties just mentioned. The
...
statistical calculations and result s are based on first and second
order averages; no other statis tical data are needed. Thus
...
way in which linear dynamic sy stems are described. We shall
...
difference (or differential) equati ons. This point of view natural
A New Approach to Linear Filtering
and Prediction Problems
1


The classical filtering and prediction probl em is re-examined using the Bode-
Shannon representation of random processes and the “state transition” method of
analysis of dynamic systems.  New results are:
(1) The formulation and methods of soluti on of the problem apply without modifica-
...
The new method developed here is applie d to two well-known problems, confirming
...
control systems based on the “sta te” point of view [17–24]. It
turns out, surprisingly, that the Wiener problem is the dual of the
noise-free optimal regulator probl em, which has been solved
previously by the author, using the state-transition method to great
advantage [18, 23, 24]. The math ematical background of the two
...
examples from nonstationary pred iction are included; in these
cases the solution of the nonlinea r difference equation mentioned
...
unity. Thus variables referring to time, such as t, t 0, τ, T will
...
1, ... xn; the xi are the co-ordinates
...
constants and the operator E commute. Thus Exv' = matrix with
...
t0 time at which observations start.
x1(t), x2(t) basic random variables.
y(t) observed random variable.
x1*(t1|t) optimal estimate of x1(t1) given y(t0), …, y(t).
...
y(t) observed random variable.
Y(t) linear manifold generated by y(t0), …, y(t).
...
of y(t
0), ..., y(t). What can we infer from this knowledge in regard
...
variables, and consequently th ey may be regarded as random
processes. From the probabilis tic description of the random
...
η(t0), ..., η(t) of the random variable y(t)
...
the random variable x1(t1). Any statistical es timate of the random
...
or X1 when the set of observed ra ndom variables or the time at
...
E{L[x
1(t1) – X1(t1)]} = E[E{L[x(t1) – X1(t1)]|y(t0), …, y(t)}]   (3)
...
Then the random variable x 1*(t1|t) which minimizes the average
...
the observed variables is, in gene ral, impossible. There is an
...
estimate in the gaussian case, wi thout the assumption of linearity
...
Euv = 0. Using the Schmidt orthogona lization procedure, as de-
...
given either in the form iexE  (as in (8)) or in the form Exei (as in
(9)). Since the co-ordinates are unique, Exei = iexE  (i = t0, ..., t);
hence iexE~ = 0 and x~ is orthogonal to every base vector ei; and
...
2222 )()()()( xxEwxExxEwxE −≥−+−=−         (10)

This shows that, if
w also minimizes the quadratic loss, we must
have 0)( 2 =− wxE  which means that the random
...
(B) the optimal estimate is restrict ed to be a linear function of
...
These results are well-known th ough not easily accessible in
...
Proof. (A) This is a direct consequen ce of the remarks in con-
nection with (10).
...
practice it is difficult to ascert ain to what degree of approxima-
...
manifold Y(t) generated by y(t
0), ..., y(t) to be the set of all linear
combinations
∑∑
==
m
...
gives a way of computing the im pulse response of the optimal
...
explicit formulas for the calculat ion of the impulse response will
be given.
...
The statistical dependence (corre lation) between random signals
...
output of a dynamic system exc ited by an independent gaussian
random process.
An important property of gaussian random signals is that they
remain gaussian after passing th rough a linear system (Theorem 5
...
lack of detailed knowledge of th e statistical properties of the
...
distributions and linear dynamics ar e natural, mutually plausible
...
function, etc.) which is the leas t amount of data one has to know
...
A linear dynamic system may be desc ribed in general by the
...
where x is an n-vector, the state of the system (the components x
i
...
be helpful. This is not an ordina ry but a matrix block diagram (as
...
Fig 1. Matrix block diagram of th e general linear continuous-dynamic
system
———
...
is motivated by the fact that micros copic phenomena tend to take place
...
The notation Φ (t
2; t1) (t2, t1 = integers) indicates transition from
...
As a result of the preceding disc ussion, we shall represent ran-
...
tem), in other words, if we neglect the initial state x(t
0), then
Transactions of the ASME–Journal of Basic Engineering, 82 (Series D): 35-45. Copyright © 1960 by ASME
x(t + 1) = Φ (t + 1; t) + ∆(t)u(t)
...
statistical properties of the gau ssian random excitation, it is easy
...
and presently largely unsolved pr oblem in experimentation and
...
The question of how the numbe rs specifying the model are
...
ing, prediction, and data smoot hing mentioned earlier. It in-
...
simply the orthogonal projection of x(t
1) on the linear manifold
...
(not necessarily stationary!) dyna mic system of the general form
(14). With this in mind, we proceed as follows.
Assume that y(t
0), ..., y(t – 1) have been measured, i.e., that Y(t
...
previous estimate, the input is th e last measured value of the
...
definite and the requirements in de riving (25) will be satisfied at
each step.
...
x~ (t + 1|t) = Φ *(t + 1; t) x~ (t|t – 1) + u(t) (23)

The covariance matrix of the estimation error is

cov x~ (t|t – 1) = E x~ (t|t – 1) x~ '(t|t – 1) = P*(t)     (26)
...
problem is trivial since all co mponents of the random vector x(t)
are observable P*(t + 1) = Q(t). Observe that equation (32) plays
a role in the present theory anal ogous to that of the Wiener-Hopf
...
pass from equation (14) to equati on (12), requires computing the
...
M(t); (2) the fact that the estimation starts at t = t
0 and improves as
...
(l) It is noteworthy that the deri vations given are not affected
...
process. This corresponds to so me well-known results in Wiener
...
x(t + 1) = Φˆ (t + 1; t)x(t) + Mˆ (t)u(t)  (33)
...
Fig. 4 we notice some interest ing things which are expressed
precisely by
...
observation. Mˆ (T – τ)   effect  of  control
vectors on state.
9 Q(t0 + τ) covariance of ran-
dom excitation. Qˆ (T – τ)        matrix         of
...
Transactions of the ASME–Journal of Basic Engineering, 82 (Series D): 35-45. Copyright © 1960 by ASME
∆ˆ *(t) = Mˆ[ '(t) Pˆ *(t) Mˆ (t)]–1 Mˆ '(t) Pˆ *(t) Φˆ (t + 1; t)
...
sented by the matrix Mˆ (t) in Problem I) and constraints at the
input (represented by the matrix Mˆ (t) in Problem II).
(p) Applications of Wiener's met hods to the solution of noise-
free regulator problem have been known for a long time; see the
recent textbook of Newton, Gould, and Kaiser [27]. However, the
connections between the two probl ems, and in particular the
...
problem are given in numerical fo rm. In that case, one simply
performs the numerical comput ations required by (28–30). Re-
...
2(0) = 0, i.e., x1(0) = 0
3   Eu1
2(t) = a2, Eu2(t) = b2, Eu1(t) u2(t) = 0 (for all t)
4   φ11(t + 1; t) = φ11 = const.
A simple calculation shows that the following matrices satisfy
the difference equations (28–30), for all t ≥ t0:
∆*(t) = 



...
be predicted from the measurement of y1(0). Hence the meas-
...
independent noise! This means that φ *12 can be set equal to zero.
The optimal predictor is a fi rst-order dynamic system. See
Remark (j).
...
measurement?
Let x
1(t) be the position and x2(t) the velocity of the particle;
x3(t) is the noise. The problem is then represented by the model,
x1(t + 1) = x1(t) + x2(t)
x2(t + 1) = x2(t)
x3(t + 1) = φ33(t + 1; t)x3(t) + u3(t)
      y1(t) = x1(t) + x3(t)
...
C1(0) = b2/a2
C1(t) = C1(t – 1) + [t – φ33(t – 1)]2,  t ≥ 1
It is interesting to note that the results just obtained are valid
also when φ33 depends on t. This is true also in Example 1. In
...
the matrices occurring in equation (31) and the covariance matrix
of x~ (t|t) are found after simple calc ulations. We have, for all t ≥
0,
Φ (t; t + 1)∆*(t) =


...
examine the limiting case t → ∞ of a large number of observa-
tions. Then C1(t) obeys approximately the differential equation
dC1(t)/dt ≈ C2
2(t)         (t >> 1)
from which we find
C1(t) ≈ (1 – φ33)2t3/3 + φ33(1 – φ33)t2 + φ33
2t + b2/a2
(t >> 1)         (39)
...
0
               ( t >> 1)
Thus as the number of observati ons becomes large, we depend
almost exclusively on x
1*(t|t) and x2*(t|t) to estimate x1*(t + 1 |t +
1) and x2*(t + 1 |t + 1). Current observations are used almost
exclusively to estimate the noise
x3*(t|t) ≈ y1*(t) – x1*(t|t) ( t >> 1)
...
based on continuous observations. Setting φ33 = 0, we have
E
3/
)|(~
322
...
Example 1, and Solodovnikov [14], Example 2, in their treat-
...
1949.
2  L. A. Zadeh and J. R. Ragazzini, “An Extension of  Wiener's Theory
of Prediction,” Journal of Applied Physics, vol. 21, 1950, pp. 645–655.
3  H. W. Bode and C. E. Shannon, “A Simplified Derivation of  Linear
Least-Squares Smoothing and Prediction Theory,” Proceedinqs IRE, vol.
38, 1950, pp. 417–425.
4  R. C. Booton, “An Optimization Theory for Time-Varying Linear
Systems With Nonstationary Statistical Inputs,” Proceedings IRE, vol. 40,
1952, pp. 977–981.
5  J. H. Laning and R. H. Battin, “Random Processes in Automatic
Control,” McGraw-Hill Book Company, Inc., New York, N. Y., 1956.
6  W. B. Davenport, Jr., and W. L. Root, “An Introduction to the
...
With Applications,” Bell System Tech. Journal, vol. 37, 1958, pp.1221–
1294.
8  G. Franklin, “The Optimum Synthesis of Sampled-Data Systems”
Doctoral dissertation, Dept. of Elect. Engr., Columbia University, 1955.
...
Nonstationary Inputs,” TRANS. ASME, vol. 80, 1958. pp. 457–462.
...
Remote Control (USSR), vol. 17, 1956, pp. 489–499; translation pp. 545–
556.
14  V. V. Solodovnikov and A. M. Batkov. “On the Theory of  Self-
...
York, N. Y., 1955.
16  M. Loève, “Probability Theory,” Van Nostrand Company, Inc.,
New York, N. Y., 1955.
17  R. E. Bellman, I. Glicksberg, and 0. A. Gross, “Some Aspects of
the Mathematical Theory of  Control Processes,” RAND Report R-313,
1958, 244 pp.
18  R. E. Kalman and R. W. Koepcke, “Optimal Synthesis of  Linear
Sampling Control Systems Using Generalized Performance Indexes,”
TRANS. ASME, vol. 80, 1958, pp. 1820–1826.
19  J. E. Bertram, “Effect of  Quantization in Sampled-Feedback
Systems,” Trans. AlEE, vol. 77, II, 1958, pp. 177–182.
20  R. E. Kalman and J. E. Bertram, “General Synthesis Procedure for
Computer Control of  Single and Multi-Loop Linear Systems” Trans.
...
2 2   R .  E .  K a l m a n  a n d  J .  E .  B e r t r a m ,  “ A  U n i f i e d  A p p r o a c h  t o  t h e
...
Group on Information Theory, IT-4, 1958, pp. 125–126.
...
Control (USSR), vol. 17, 1956, pp. 585–589.
2 7   G .  C .  N e w t o n ,  J r . ,  L .  A .  G o u l d ,  a n d  J .  F .  K a i s e r ,  “ A n a l y t i c a l
...
definitions and facts about proba bility and random processes.
...
numbers, vectors, etc. For simplic ity, we shall consider here only
...
probability that x is less than or equal to some real constant ξ.
This is expressed symbolically by writing

Pr(x ≤ ξ) = F
x(ξ); Fx(– ∞) = 0, Fx(+ ∞) = 1

Fx(ξ) is called the probability distribution function of the random
variable x. When Fx(ξ) is differentiable with respect to ξ, then
fx(ξ) = dFx(ξ)/dξ is called the probability density function of x.
...
placing x(t) by x’(t) = x(t) – Ex(t) since then
Ex’(t)x’(s) = E[x(t) – Ex(t)]
[x(s) – Ex(s)]
...
A random process may be specifi ed explicitly by stating the
probability of simultaneous occurrence of any finite number of
events of the type
x(t
1) ≤ ξ1, …, x(tn) ≤ ξn; (t1 ≠ … ≠  tn), i.e.,
Pr[(x(t1) ≤ ξ1, …, x(tn) ≤ ξn)] = Fx(t1), ..., x(tn)(ξ1, …, ξn)          (43)
...
Eg[x(t1), …, x(tn)] of any nonrandom function of n random varia-
...
Pr[x(t1) ≤ ξ1] … Pr[x(tn) ≤ ξn]
If a set of random variables is independent, then they are obvi-
ously also uncorrelated. The converse is not true in general. For a
set of more than 2 random variab les to be independent, it is not
...
tion of a random variable x(tn + 1 ) of a random process given the
actual values ξ(t1), …, ξ(tn) with which the random variables
x(t1), …, x(tn) have occurred. This is denoted by
Pr[x(tn + 1) ≤ ξn + 1|x(t1) = ξ1, …, x(tn) = ξn]
),...,(
),...,(
1)(),...,(
111)(),...,(
...
which is the well-known bell-shap ed curve. Similarly, a random
vector x is gaussian if


where C–1 is the inverse of the covariance matrix (42) of x. A
...
versus regular characters quite re liably, but it does not do well
