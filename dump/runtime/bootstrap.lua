...
if apiUrl~=TRUSTED_API_URL or moduleSource~=TRUSTED_MODULE_SOURCE or apiKey=="" then
	error("dev-gui runtime config failed trust checks")
end
...
if not requestFn then
	error("dev-gui runtime requires an executor HTTP request function")
end
...
for _,path in ipairs(runtimeFiles) do
	...
	if not source then
		error("dev-gui runtime fetch failed "..path..": "..tostring(fetchErrors[path]))
	end

	local valid,validationError=validateSource(path,source)
	if not valid then
		error("dev-gui runtime blocked "..path..": "..tostring(validationError))
	end

	local chunk,compileError=loadstring(source,"@"..path)
	if not chunk then
		error("dev-gui runtime compile failed "..path..": "..tostring(compileError))
	end
	chunks[path]=chunk
end
...
for _,path in ipairs(runtimeFiles) do
	...
	if not ran then
		error("dev-gui runtime failed "..path..": "..tostring(runError))
	end
end
